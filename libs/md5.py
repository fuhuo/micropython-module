...
def blockDivide(block, chunks):
    ...
    for i in range(0, chunks):
        ...
        result.append( struct.unpack('<I', block[i*size:(i+1)*size])[0])
    return(result)
...
def fmt8(num):
    ...
    # result1 = "{0:08x}".format(int.from_bytes(binver,byteorder='little'))
    # print(binver)
    result = "{0:08x}".format(struct.unpack('<I', binver)[0])
    # print(f'result1: {result1} result: {result}')
    return(result)
...
def md5sum(msg):
    ...
    zeroPad //= 8
    # print(msgLen, bytes(msgLen))
    # print(msgLen.to_bytes(8,byteorder='little'))
    # msg = msg + b'\x00'*zeroPad + msgLen.to_bytes(8,byteorder='little')
    msg = msg + b'\x00'*zeroPad + struct.pack('<q', msgLen)
    msgLen = bitlen(msg)
